 ...
 *
 * Descricao: Tratamento de erros/Implementacao main/chamada ObterMatrizTransposta
 *
 * $Author: felipe.farias $
 * $Date: 2024/06/10 01:35:14 $
 * $Log: aula0504.c,v $
 * Revision 1.1  2024/06/10 01:35:14  felipe.farias
 * Initial revision
 *
 *
 ************************************************************************************/
...
{	
	...
	static ld matrizOriginal[NUMERO_MAXIMO_LINHAS][NUMERO_MAXIMO_COLUNAS], matrizTransposta[NUMERO_MAXIMO_LINHAS][NUMERO_MAXIMO_COLUNAS];
	...
	for(auxiliarLinha = 0; auxiliarLinha < numeroLinhas; auxiliarLinha++)
	{
		for(auxiliarColuna = 0; auxiliarColuna < numeroColunas; auxiliarColuna++)
		{
			...
			matrizOriginal[auxiliarLinha][auxiliarColuna] = segundoTermoAuxiliar;
				
		}
	}
	
	printf("\n%sMatriz Original:%s\n", GREEN, RESET);
	resultado = ExibirMatriz(numeroLinhas, numeroColunas, matrizOriginal);

	printf("%sMatriz Transposta:%s", GREEN, RESET);
	resultado = ObterMatrizTransposta(numeroColunas, numeroLinhas, matrizOriginal, matrizTransposta);
	resultado = ExibirMatriz (numeroColunas, numeroLinhas, matrizTransposta);
