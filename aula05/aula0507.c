 ...
 * $Date: 2024/06/18 23:47:28 $
 * $Log: aula0507.c,v $
 * Revision 1.2  2024/06/18 23:47:28  felipe.farias
 * inclusao de nova variavel
 *
 * Revision 1.1  2024/06/18 14:07:16  felipe.farias
 * Initial revision
 *
 *
 *******************************************************************************/
...
{	
	...
	ld complementoAlgebrico[1];
	char *validacao;
	unsigned char resultadoComplementoAlgebrico;

	/* NUMERO DE ARGUMENTOS DA MATRIZ CALCULAR COMPLEMENTO ALGEBRICO NO TERMINAL */
	...
	/* TRATAMENTO DO INDICE LINHA DO ELEMENTO DA MATRIZ */
	...
	/* TRATAMENTO DO INDICE COLUNA DO ELEMENTO DA MATRIZ */
	...
	/* INDICES DA MATRIZ QUE SERA USADA PARA CALCULAR O COMPLEMENTO ALGEBRICO */
	...
	resultadoComplementoAlgebrico = CalcularComplementoAlgebrico (ordemMatriz, indiceLinhas, indiceColunas, matriz, complementoAlgebrico);
	
	if (resultadoComplementoAlgebrico == linhaColunaInvalida)
	{
	...
	printf("%sIndice referencia para calcular complemento algebrico: a[%hu][%hu] =%s %.5Lf\n", GREEN, (us)indiceLinhas, (us)indiceColunas, RESET, matriz[indiceLinhas - 1][indiceColunas - 1]);
	printf("\n%sComplemento Algebrico =%s %.5Lf\n\n", GREEN, RESET, complementoAlgebrico[0]);
